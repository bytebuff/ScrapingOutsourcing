...
def readViewDetail():
    with open('第三次采集目标名单.txt') as fp:
        for word in fp:
            yield word.strip()

# 初始化网址
start_urls = [f'http://cmispub.cicpa.org.cn/cicpa2_web/07/{word}.shtml' for word in readViewDetail()]
...
# 管道文件(pipelines管道)
def pipeline(item):
    with open('第三次采集最终结果.csv', 'a', encoding='utf-8', newline='') as fp:
